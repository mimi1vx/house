 ...
 * `H.VirtualMemory` is aarch64 4 KiB-granule L0→L3 over `0x01000000–0x3FFFFFFF` (TTBR0=user, per-`PageMap` `TTBR0_EL1`+ASID via `userspace.c:house_handle_user_fault` demand pager that allocates 4K via `buddy` and `VAE1IS`+SGI 1 shootdown). Kernel RAM `0x40000000`+`house_ram_bytes` stays TTBR1 Normal WB Inner-shareable (`SCTLR_EL1.C/I=1`).
