...
* `Kernel.FileSystem.Vfs` provides longest-prefix mount routing and per-process namespaces. Namespace 0 mounts volatile `RamFs` at `/`; fork inherits a namespace descriptor, and newly opened paths resolve through the process namespace. Inherited descriptors retain the namespace recorded when they were opened. `BlkFs` implements an HFS1/virtio-blk backend. The shell's `blk sync` and `blk mount` commands persist and restore the default RamFS rather than replace its mount, while `blk root <slot>` decodes an HFS1 image completely and then appends it as a lower `/` layer. Dynamic dependency reads walk the layers of the longest matching prefix, so the initramfs RamFS stays the upper layer and wins first; a lower layer is consulted only on `ENOENT`, any other backend error fails closed, a second lower root is refused, and an attached root blocks block-server teardown. Each layer records the digest of its own `/lib` content in `/lib/.house-lib-version`, derived from that layer's files rather than hand-written, with paths spelled layer-relative so identical content yields identical digests. A dynamic exec compares the two records and logs one `dmesg` line whenever they are not the same digest, `lib version skew: initramfs=<12 hex> block-root=<12 hex>`, shortened to 12 hex per side so the line always fits the 120-column ring. The check is diagnostic only and never changes which layer serves a dependency, so the initramfs copy still wins every lookup, and it is skipped entirely when a single layer is mounted. A missing, unreadable, or malformed record is reported as `unpinned` on that side rather than ignored, since an image predating the record is the likeliest real skew. Ordinary reads, writes, `ls`, and `stat` stay upper-layer only. `H.FileSystem` remains a compatibility shim over the default namespace. RamFS starts with the 512-page legacy pool and falls back to buddy pages; it has no separate 2 MiB quota. Whole `PT_LOAD` segments are still mapped into RAM per mapping process, so a large shared object costs RAM in every process that maps it until demand paging lands.
...
`ET_DYN` images run the same path. `libc-house.so.0` ships in the initramfs `/lib`, and `hello-dyn` links against it through the pinned `/lib/ld-house.so.0` interpreter string, which is recorded and validated but never opened and has no staged artifact. A dependency graph is walked depth-first in `DT_NEEDED` discovery order, each name resolved by SONAME alone and read initramfs-first through the layer stack, and each object then placed at its own page-aligned base. `/bin/hello-dyn-deep` is the three-object probe: it needs `libmid-house.so.0`, which itself needs `libc-house.so.0`, so the plan carries a relocation inside a non-main object whose provider is the sibling DSO and a main symbol resolved from the deepest object. That plan is asserted twice-built and byte-compared by `make dynamic-elf-check`, and the QEMU stage now proves the same relocation at EL0: the deep probe calls `house_len`, which reaches `libc-house.so.0` through `libmid-house.so.0`'s own JUMP_SLOT, so the probe cannot pass without a correct non-main GOT read. `loaderrefs /bin/hello-dyn-deep` additionally shares the finalized read-only pages of two dependencies across two live images.
...
make house-dynamic-root-check  # -> offline ENOENT + dmesg, mounted-root /lib fallback, initramfs-first lookup, lib version skew dmesg, teardown guard (hvf+tcg)
