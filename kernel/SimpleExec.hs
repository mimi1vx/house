...
import H.IOPorts
import H.Interrupts (IntId, enableInt, eoi, ppiVirtTimer)
import H.Monad (H)
import H.PhysicalMemory (PAddr, PhysPage, allocPhysPage, pageSize, setPAddr)
import H.UserMode (Context (..), Interrupt (..), execContext)
import H.VirtualMemory (PageInfo (..), PageMap, VAddr, allocPageMap, setPage)
...
fixPage uproc vaddr
  | vaddr >= (startCode uproc) && vaddr < (endCode uproc) =
      let codeOffset = fromIntegral ((pageFloor vaddr) - (startCode uproc))
       in do
            setupPage uproc vaddr (drop codeOffset (codeBytes uproc)) False
            return True
fixPage uproc vaddr
  | vaddr >= (startData uproc) && vaddr < (endData uproc) =
      let dataOffset = fromIntegral ((pageFloor vaddr) - (startData uproc))
       in do
            setupPage uproc vaddr (drop dataOffset (dataBytes uproc)) True
            return True
fixPage uproc vaddr
  | (vaddr >= (startBss uproc) && vaddr < (brk uproc))
      || (vaddr >= (startStack uproc) && vaddr < (endStack uproc)) =
      do
        setupPage uproc vaddr (repeat 0) True
        return True
...
initTimer msPerTick =
  do
    outB timerPort (ticks .&. 0xff) -- set timer interrupt frequency (x86 PIT, no-op on aarch64)
    outB timerPort (ticks `shiftR` 8)
    enableInt ppiVirtTimer
  where
    ticks = fromIntegral (1193190 * msPerTick `div` 1000)
...
exec uproc context =
  do
    (interrupt, context') <- execContext (pmap uproc) context
    case interrupt of
      ...
      PageFault _ faultAddr ->
        do
          fixOK <- fixPage uproc faultAddr
          if fixOK
            then
              exec uproc context'
            else do return ("Fatal page fault at " ++ (showHex faultAddr ""))
      ExternalInterrupt irq | irq == ppiVirtTimer ->
        {- timer interrupt -}
        do
          eoi irq
