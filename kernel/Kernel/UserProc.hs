...
import H.Interrupts (IntId, eoi, ppiVirtTimer)
import H.Monad
  ( H,
  )
import H.PhysicalMemory (PhysPage, allocPhysPage, freePhysPage, pageSize, setPAddr)
import H.UserMode (Context (..), Interrupt (..), execContext)
import H.VirtualMemory (PageInfo (..), PageMap, VAddr, allocPageMap, freePageMap, setPage)
import Kernel.AOut
  ...
  do
    ...
    case interrupt of
      ...
      PageFault _ faultAddr ->
        do
          r <- fixPage uproc' faultAddr
          case r of
            Left uproc'' -> execUProc uproc''
            Right msg -> exitWith msg
      ExternalInterrupt irq | irq == ppiVirtTimer -> -- timeslice exhausted
        do
          -- eoi irq
...
doSyscall uproc@UProc {context = Context {eax = callnum, ebx = arg}} =
  do
    ...
    case callnum of
      ...
      1 ->
        {- putChar -}
        do
          myPutStr uproc [chr (fromIntegral (arg .&. 0xff))]
  ...
  where
    withResult uproc@UProc {context = context} v =
      Left (uproc {context = context {eax = v}})

freeUProc :: UProc -> H ()
freeUProc uproc =
  do
    freePageMap (pmap uproc)
...
extendBrk uproc incr =
  let oldBrk = brk uproc
      newBrk =
        ( fromIntegral
            ( ((fromIntegral oldBrk) :: Int)
                + ((fromIntegral incr) :: Int)
            )
        ) ::
          Word32
   in if newBrk `inVRegion` (oldBrk, maxBrk uproc)
        then
...
fixPage uproc vaddr
  | vaddr `inVRegion` (codeRegion $ aout uproc) =
      setupPage
        uproc
        vaddr
        False
...
fixPage uproc vaddr
  | vaddr `inVRegion` (dataRegion $ aout uproc) =
      setupPage
        uproc
        vaddr
        True
...
fixPage uproc vaddr
  | vaddr `inVRegion` (fst $ bssRegion $ aout uproc, brk uproc) =
      setupPage uproc vaddr True zeroSrc
fixPage uproc vaddr
  | vaddr `inVRegion` (stackRegion uproc) =
      setupPage uproc vaddr True zeroSrc
fixPage _ vaddr
  | otherwise =
  ...
  do
    ...
    case pagem of
      ...
      Nothing ->
        return $
          Right
            ( "Out of pages when servicing fault at "
                ++ (showHex vaddr "")
            )
...
copyPage physPage src =
  sequence_
    [ setPAddr (physPage, off) =<< src (fromIntegral off)
    | off <- [minBound ..]
    ]
...
pageCeiling vaddr =
  ((vaddr - 1) `div` (fromIntegral pageSize) + 1)
