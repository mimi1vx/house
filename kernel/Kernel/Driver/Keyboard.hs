    ...
    let set = x86Set1
        ...
        scanHandler cs =
          do
            c <- readChan codeChan
            let entrySeq = cs ++ [c]
            case lookup entrySeq set of
              Nothing ->
                if length entrySeq >= longestSequence
                  then do
                    -- putStrLn "Unknown scan code sequence"
                    writeChan
                      keyChan
                      (UnknownKeyEvent entrySeq)
                    scanHandler []
                  else scanHandler entrySeq
              Just event -> do
                writeChan keyChan event
                scanHandler []
    _ <- forkH $ scanHandler []
    return keyChan
  ...
  do
    ...
    let interpreter mods dead =
          do
            ...
            writeList2Chan pressChan ks
            interpreter mods' dead'
    _ <- forkH $ interpreter Set.empty Nothing
    return pressChan
...
interpret mods dead (KeyPressed (Key c)) =
  ...
  where
    handleMapping (Left dead') =
      case dead of
        Nothing -> (mods, Just dead', [])
        Just ch ->
          let ks = map (KeyPress mods . Key) [ch, dead']
           in (mods, Nothing, ks)
...
x86Set1 :: KeyboardSet
x86Set1 =
  map (f KeyPressed) x86Set1Pressed
