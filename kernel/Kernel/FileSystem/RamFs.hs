...
Pure 'H.Pages' backing; 2 MiB cap, no host I/O. Single global root
protected by one 'QSem' (matches 'H.Pages.pageSem' pattern). Exposed
as 'ramfsOps' for the VFS switch; 'H.FileSystem' is a thin shim over
...
-}
ramfsWrite :: FilePath -> String -> H (Either FsError ())
              ...
              Just (Dir kids) -> case Map.lookup name kids of
                ...
                Just (File oldPages _ _) ->
                  case updateAt
                    ...
                    root of
                    Left e -> return (Left e)
                    Right nrt -> do writeRef ramfsRoot nrt; return (Right (Just oldPages))
                Nothing ->
                  case updateAt
                    parentComps
                    ( \case
                    ...
                    root of
                    Left e -> return (Left e)
                    Right nrt -> do writeRef ramfsRoot nrt; return (Right Nothing)
          case result of
            Left e -> do
...
ramfsRead :: FilePath -> H (Either FsError String)
...
ramfsReadBytes :: FilePath -> H (Either FsError [Word8])
ramfsReadBytes path = case splitPath path of
  Left e -> return (Left e)
  Right cs -> withQSem ramfsSem $ do
    root <- readRef ramfsRoot
    case resolve cs root of
          ...
          Just (Dir kids) -> case Map.lookup name kids of
            ...
            Just f@(File {}) ->
              case updateAt
                ...
                root of
                Left e -> return (Left e)
                Right nrt -> do writeRef ramfsRoot nrt; return (Right (Right f))
      case res of
        Left e -> return (Left e)
