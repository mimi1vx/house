...
make smp-check-8                         # scaling gate: SMP_N=8 at 4G (ceiling 16; N=8 hvf flaky — nightly gate, N=2 per-commit)
make vm-check                            # demand paging + mprotect/munmap + ASID + shootdown  512M/2+4G/4 hvf+tcg + mem buddy free/total (pressure leg)
...
- **RAM auto-detected.** No `HOUSE_RAM_BYTES` limit. DTB `reg` (via `x0`) → fault probe `128M→16G` → `512M` fallback; same binary boots at any QEMU `-m` without rebuild. `-kernel` must be the flat `build/*.bin` (`objcopy -O binary`): QEMU boots non-ELF aarch64 images via its Linux path (`x0`=DTB); ELF `-kernel` gets `x0=0`, no DTB, and the fault probe false-positives on hvf. `SPIKE_MEM`/`HOUSE_RAM_LIMIT` (default `4G`) only drives `qemu -m` and caps via `HOUSE_RAM_LIMIT_BYTES` (`min(detected, limit)`); `SMP_N`/`HOUSE_SMP_LIMIT` (default `2`, max `16`, tested to `8`) sets per-core 64 KiB stacks (`house_boot_stack_top - core*64K`).
- **MMU/buddy/EL0.** Split `TTBR1` kernel / `TTBR0` user `0x01000000–0xFFFFFFFF` (`TCR T1SZ=16 TG1=4K`), 8-bit ASID with `VMALLE1IS` wrap; demand pager `house_handle_user_fault` + RO perm faults via `house_is_ro_page`; shootdown `VAE1IS` + SGI 1. Buddy manages `__heap_base+64M .. house_boot_stack_top-16*64K` (see `rust/crates/house-hal-aarch64/src/buddy.rs`). EL0 via `svc #imm` (`WRITE 0x01`/`EXIT 0x02`/`IPC 0x10..0x14`), `house_enter_el0` `eret`. Shell: `free`/`mem`/`detect`/`vm` show `buddy`/`ram`/`TTBRs`/`TCR`.
