...
- `scripts/` — 6 `expect` harnesses (`qemu-smoke.exp`, `qemu-irq.exp`, `qemu-house.exp`, `qemu-house-shell.exp`, `qemu-house-posix.exp`, `qemu-smp.exp`). ELF path is argv.
...
make smp-check                # smp: 2 cores online + caps 2 + parfib 20=6765 + mvar ok hvf+tcg
...
- **RAM is compiled in.** `SPIKE_MEM` (default `4G`) computes `SPIKE_RAM_BYTES` + `BOOT_STACK_TOP`/`HOUSE_RAM_BYTES`/`HOUSE_STACK_TOP` in `Makefile:22-35` and drives both `DEFS_C/S` and QEMU `-m`. `SPIKE_MEM` and QEMU `-m` can never disagree. `SMP_N` (default `2`, max `4`) computes `HOUSE_SMP_N` and per-core 16K stacks (`BOOT_STACK_TOP - core*16K`, `__early_stacks_base + SMP_N*16K`); `SPIKE_MEM`+`SMP_N` together must fit (512M+2 stacks is smallest gate). Small values are first-class.
...
- **GICv3** (`gic.c`): `GICD 0x08000000`, `GICR 0x080A0000 + core*0x20000`; wake `GICR_WAKER` per core, mark PPIs 27/29/30 + SGI 0 Group1 per core, enable via `ICC_PMR/IGRPEN1/BPR1`. `house_gic_send_sgi(0, 1<<core)` via `ICC_SGI1R_EL1` kicks remote core's scheduler. `H.Interrupts` maps `ppiVirtTimer=27`, `ppiPhysTimer=30`, `spi n = 32+n`.
