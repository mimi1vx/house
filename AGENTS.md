...
- **RAM auto-detected.** No `HOUSE_RAM_BYTES` limit. DTB `reg` → probe `128M→16G` → `128M` fallback; same ELF boots at any QEMU `-m` without rebuild. `SPIKE_MEM`/`HOUSE_RAM_LIMIT` (default `4G`) only drives `qemu -m` and caps via `HOUSE_RAM_LIMIT_BYTES`; `SMP_N`/`HOUSE_SMP_LIMIT` (default `2`, max `16`, tested to `8`) sets per-core 64 KiB stacks (`house_boot_stack_top - core*64K`).
