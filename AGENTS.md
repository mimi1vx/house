...
- `scripts/` — 9 `expect` harnesses (`qemu-smoke.exp`, `qemu-irq.exp`, `qemu-house.exp`, `qemu-house-shell.exp`, `qemu-house-posix.exp`, `qemu-smp.exp`, `qemu-ipc.exp`, `qemu-driver.exp`, `qemu-virtio-transport.exp`). ELF path is argv.
